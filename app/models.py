...
class Material(db.Model):
    ...
    Unit = db.Column(db.String(20), nullable=False) # kg, gr, unidades, etc.
...
class ProductionTask(db.Model):
    ...
    AssignedTo = db.Column(db.Integer, db.ForeignKey('users.iD_User'), nullable=True)
    Status = db.Column(db.String(50), nullable=False, default='Pendiente')
    TargetQuantity = db.Column(db.Integer, nullable=False)
    ProducedQuantity = db.Column(db.Integer, nullable=False, default=0)

    # Relación para obtener el nombre del usuario
    assigned_user = db.relationship('Users', backref='production_tasks', lazy=True)
    ...
    def to_dict(self):
        return {
            ...
            'ProducedQuantity': self.ProducedQuantity
        }
